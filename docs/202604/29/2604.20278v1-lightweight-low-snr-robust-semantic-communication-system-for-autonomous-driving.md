...
tags: ["query:cv"]
score: 9.0
evidence: 基于批归一化和L1正则化的结构化剪枝，用于降低模型复杂度
tldr: 针对自动驾驶中车联网协作感知的资源受限和低信噪比鲁棒性差等挑战，本文提出一种轻量化且鲁棒的深度联合源信道编码（JSCC）语义通信系统。通过结构化剪枝大幅降低模型复杂度，并结合均匀量化与M-QAM调制实现与现有数字通信系统的兼容。实验证明，该系统在大幅减少参数量的同时，在低信噪比环境下表现优于传统通信方案。
source: arxiv
...
result: 剪枝后的模型在减少超过一半参数的情况下，仍保持了良好的重建质量，且在低信噪比下显著优于传统方法。
conclusion: 该系统实现了轻量化与高性能的平衡，为低信噪比环境下的自动驾驶语义通信提供了有效的数字化兼容方案。
---
