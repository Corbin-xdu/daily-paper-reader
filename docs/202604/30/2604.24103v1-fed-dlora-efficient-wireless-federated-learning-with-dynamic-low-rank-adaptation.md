...
tags: ["query:cv"]
score: 6.5
evidence: 车联网中高效联邦学习的动态低秩自适应
tldr: 针对车联网中联邦学习面临的通信开销大和环境动态变化等挑战，本文提出了一种名为Fed-DLoRA的轻量级算法。该方法结合低秩自适应（LoRA）技术，通过动态调整秩、带宽和车辆选择来降低参数量和通信成本。研究通过收敛性分析建立了秩与性能的关系，并设计了ARBVS算法进行资源调度，显著提升了训练效率和模型精度。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2604.24103v1/fig-001.webp\", \"caption\": \"\", \"page\": 3, \"index\": 1, \"width\": 816, \"height\": 667}]"
motivation: 旨在解决车联网环境下联邦学习的高通信开销以及动态环境带来的资源调度难题。
method: 提出结合LoRA的Fed-DLoRA算法，并利用ARBVS算法动态优化秩选择、带宽分配和车辆参与策略。
result: 实验表明，该方法在提高模型准确率的同时，显著加快了收敛速度并降低了通信负担。
conclusion: Fed-DLoRA通过动态低秩自适应和联合资源优化，为高效、可靠的无线联邦学习提供了有效的解决方案。
---

## 摘要
联邦学习（FL）为车联网（IoV）应用提供了一种极具前景的分布式学习范式。然而，它面临着通信开销和动态环境带来的挑战。模型压缩技术虽然减轻了计算和通信负担，但在压缩率与车辆参与策略之间产生了权衡。在本文中，我们提出了一种名为动态低秩自适应联邦学习（Fed-DLoRA）的轻量级联邦学习算法，该算法结合了低秩自适应（LoRA），在提高训练效率的同时，有效减少了参数量和通信成本。通过结合随机梯度下降优化与奇异值分解，对 Fed-DLoRA 进行了收敛性分析。该分析建立了 LoRA 秩、车辆调度策略与模型收敛特性之间的理论关系。基于这些见解，我们制定了一个旨在最大化系统性能的联合优化问题。为了解决这一问题，我们提出了一种自适应秩、带宽和车辆选择（ARBVS）算法，该算法集成了枚举与贪婪优化策略。该算法为每个联邦学习通信轮次提供高效的秩选择和资源调度策略，从而实现了联邦学习系统性能的有效提升。实验结果表明，与传统的联邦学习方法相比，Fed-DLoRA 具有更优越的性能，表现出更高的准确性、更快的收敛速度以及更高的通信效率。
