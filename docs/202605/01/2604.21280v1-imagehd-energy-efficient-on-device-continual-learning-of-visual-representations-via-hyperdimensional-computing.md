...
tags: ["query:cv"]
score: 8.5
evidence: 用于轻量化设备端视觉表示学习的高维计算
tldr: 针对边缘AI设备在非平稳数据流中进行持续学习时面临的高计算和内存开销问题，本文提出了ImageHD。该系统结合了超维计算（HDC）的快速非迭代更新特性与量化CNN特征提取器，并设计了硬件感知的聚类管理策略。通过在FPGA上实现流式数据流架构，ImageHD在保持高精度的同时，显著提升了处理速度和能效，为资源受限硬件上的实时持续学习提供了高效方案。
source: arxiv
