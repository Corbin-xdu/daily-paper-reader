...
tags: ["query:cv"]
score: 7.5
evidence: 在严格计算限制下的真实平台高效部署
tldr: 本研究针对具身智能在实际机器人部署中面临的计算资源受限与推理能力不足的矛盾，提出了一种可部署的视觉语言导航（VLN）系统。该系统通过异步解耦感知、记忆与推理模块，构建分层认知记忆图，并将探索问题建模为上下文感知的加权旅行维修员问题（WTRP）。实验证明，该系统在保持实时性的同时，显著提升了导航的成功率与效率。
source: arxiv
...
motivation: 解决具身智能系统在受限硬件资源下，难以兼顾强大的高层推理能力与实时部署效率的挑战。
method: 采用异步模块化架构，利用认知记忆图进行分层推理，并结合WTRP算法优化上下文感知的路径探索。
result: 在仿真和真实机器人平台上均实现了优于现有方法的导航成功率和效率，且满足低功耗硬件的实时性要求。
conclusion: 该系统成功弥合了复杂视觉语言推理与嵌入式实时部署之间的鸿沟，为具身智能的实际应用提供了高效方案。
---
